            ...
            <tbody>
                <?php
                    require('koneksi.php');
                        $id = $_GET['id'];
                        $no = 0;
                        $data = mysqli_query($koneksi, "SELECT * from detail_pembayaran INNER JOIN tb_kamar ON tb_kamar.id_kamar = detail_pembayaran.id_kamar INNER JOIN tipe_kamar ON tipe_kamar.id_tipe_kamar = detail_pembayaran.id_tipe_kamar WHERE id_pembayaran = $id ");
                        while($tampil = mysqli_fetch_array($data)) { $no++;
                            
                        ?>
                    <tr>
                        <td style="text-align:center"><?php echo $no; ?></td>
                        <td style="text-align:center"><?php echo $tampil ['id_pembayaran'] ?></td>
